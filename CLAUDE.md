...
npm run insights            # Summarize logs/insights.jsonl (per-tool success, categories, clients)
```
...
36. **Hosted file delivery — signed download links** (`src/utils/download-registry.js` + `GET /files/download` in `server.js`). THE proper way a remote MCP gets a generated file onto the caller's machine: every create-* response includes a `downloadUrl` (stateless HMAC-signed token, `?token=…`, ~24h TTL, sandboxed to allowed bases, streamed with `Content-Disposition: attachment`) AND the dispatch adds an idiomatic MCP `resource_link` content block (`wrapCreateResult` in `tool-registry.js`). A human clicks it; an agent GETs it. Only active when a public base is known (`FILE_DOWNLOAD_BASE` or `PUBLIC_HOST`) — i.e. hosted; on stdio/self-host `buildDownloadUrl` returns null (file is already local). Token secret = `FILE_TOKEN_SECRET` || `PROVISION_SECRET` || admin token || ephemeral. Verified live end-to-end (download returns the real bytes).
...
35. **Learning loop: insight log + agent memory nudges** — `src/utils/insights.js` (`logInsight` → JSONL at `INSIGHTS_LOG` or `<DATA_DIR|cwd>/logs/insights.jsonl`; `memoryNudge`) and `src/utils/client-profile.js` (`setClientInfo`/`resolveClientProfile` with `MEMORY_CAPABLE` regex + `MCP_CLIENT_MEMORY` env override). All four create-* tools `logInsight` on success (always — for the creator to mine) and add a `memoryNudge` to the response ONLY when the client is a memory-capable coding agent. clientInfo is captured via `server.oninitialized` in `index.js` (stdio, one session). The HOSTED HTTP server is STATELESS so clientInfo is unknown at tool time → no nudge unless `MCP_CLIENT_MEMORY=1` — exactly right (LM Studio over the Funnel stays silent; Claude Code/Qwen/etc. over stdio get nudged). Mirrored in mcp-web-search (`client-detect.ts` `clientCanPersistMemory()` + `insights.ts` `searchLearning()`/`toolLearning()` across all 4 file/search tools on success/no-results). Nudges are **deduped per session** (a `Set` keyed by tool+category/event) so repeated identical ops don't nag — teach once. `scripts/insights-report.mjs` (`npm run insights`) summarizes the log: per-tool success rates, top failing queries, format/category mix, client breakdown.

31. **Output location follows the CALLER, via `getOutputRoot()` (`src/tools/utils.js`)** — all generated files are rooted at `DOC_OUTPUT_DIR` (env override, absolute or cwd-relative) ELSE `process.cwd()`. For a **stdio** MCP launched by an agent (Claude Code), cwd is the workspace, so files land in the project the agent was opened from. For LM Studio, set `DOC_OUTPUT_DIR` in the mcp.json `env`. `enforceDocsFolder`, `validateAndNormalizeInput`, and `applyCategoryToPath` all use it. Hosted clients can also pass a per-request **`X-Output-Dir`** header (or `?output_dir`) — carried via `requestContext` (AsyncLocalStorage) and **SANDBOXED** by `getOutputRoot()` under `CLIENT_OUTPUT_BASE` (default `<DATA_DIR>/client-output`) so a tenant can organize but not escape. **Hard truth: a REMOTE/hosted server writes to the HOST's disk** — X-Output-Dir only picks a folder ON THE SERVER; it cannot write to the calling client's machine (client/server boundary). To get files on the caller's machine, run the MCP **locally over stdio** there (DOC_OUTPUT_DIR / OUTPUT_DIR, unsandboxed). Mirrored in mcp-web-search. The hosted instance is for remote consumers (website demo, claude.ai, CogniRunner) whose files legitimately live server-side or get pushed via the upload bridge.
